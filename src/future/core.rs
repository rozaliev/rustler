...
impl<T: Send+'static, E:Send+'static> Core<T,E> {
    pub fn new() -> Core<T, E> {
        Core { inner: Box::into_raw(Box::new(Inner::new())) }
    }

    pub fn from_result(v: Result<T, E>) -> Core<T, E> {
        Core { inner: Box::into_raw(Box::new(Inner::from_result(v))) }
    }
...
impl<T: Send+'static, E:Send+'static> Inner<T,E> {
        ...
        where F: FnOnce(Result<T, E>) + 'static
    {
        match self.state() {
            New => {
                let n = self.state
                            .compare_and_swap(New.into(), NeedsResult.into(), Ordering::Relaxed)
                            .into();
                match n {
                    New => {
    ...
    pub fn set_result(&mut self, r: Result<T, E>) {
        ...
        match self.state() {
            New => {
                let n = self.state
                            .compare_and_swap(New.into(), NeedsCallback.into(), Ordering::Relaxed)
                            .into();
                match n {
                    New => {}
                    NeedsResult => {
